  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-3">
        ...
        <span className="text-indigo-400 text-sm font-medium">{year}</span>
      </div>
      <p className="text-white/80 font-medium">{institution}</p>
      ...
      {skills && (
        <div className="mt-4 flex flex-wrap gap-2">
          {skills.map((skill, index) => (
            <span
              key={index}
              className="px-3 py-1 rounded-full text-xs font-medium bg-indigo-500/10 text-indigo-300 hover:bg-indigo-500/20 transition-colors"
            >
              {skill}
            </span>
          ))}
...
function TimelineItem({ year, title, institution, description, skills }) {
  return (
    <motion.div
      ...
      transition={{ duration: 0.5 }}
    >
      <div className="absolute left-0 top-7 sm:top-8 h-4 w-4 rounded-full bg-indigo-500 border-2 border-zinc-900"></div>
      <div className="absolute left-[7px] sm:left-[7px] top-0 h-full w-[3px] bg-indigo-500/40"></div>

      <motion.div
        className="bg-white/5 backdrop-blur-sm p-6 rounded-xl border border-white/10 hover:border-indigo-500/40 transition-all duration-300"
        whileHover={{ y: -5 }}
      >
        <span className="text-indigo-400 text-sm font-medium">{year}</span>
        <h3 className="text-xl font-bold text-white mt-1">{title}</h3>
        <p className="text-white/90 font-medium mt-1">{institution}</p>
        <p className="text-gray-300 mt-3">{description}</p>
...
function SkillsSection() {
  ...
  return (
    <motion.div
      className="bg-white/5 backdrop-blur-md border border-white/10 rounded-xl p-6 mb-12"
      initial={{ opacity: 0, y: 20 }}
      whileInView={{ opacity: 1, y: 0 }}
      viewport={{ once: true }}
      transition={{ duration: 0.5 }}
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {skillCategories.map((category, index) => (
          ...
          >
            <h4 className={`text-lg font-semibold mb-3 bg-clip-text text-transparent bg-gradient-to-r ${category.color}`}>
              {category.title}
            </h4>
  ...
  return (
    <section className="p-6 md:p-20">
      ...
      <motion.h3
        ...
        transition={{ duration: 0.5 }}
      >
        <span className="bg-indigo-500 w-3 h-9 rounded mr-3"></span>
        Work Experience
      </motion.h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-16 ">
        {experiences.map((item, index) => (
          <WorkCard 
            key={index}
            year={item.year}
            title={<span className="text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-indigo-500 to-purple-500">{item.title}</span>}
      ...
      <div className="max-w-4xl mx-auto">
        <motion.h3
          className="text-2xl font-bold mb-6 mt-12 flex items-center"
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 0.5 }}
